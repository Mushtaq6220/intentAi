...
const CARDANO_SYSTEM_PROMPT = `You are a Cardano blockchain transaction parser AI.
...
1. "action":
   ...
   - "swap" for token-to-token conversions.
   - "recurring" for ongoing subscriptions or continuous payments.
   - "schedule" for single delayed future transactions.
   - "stake" for delegation of ADA to a staking pool.
   - "unstake" for withdrawing delegation.
   - "claim_rewards" for withdrawing staking rewards.
...
4. "fromToken": For swap action, identify the input token to convert FROM. Set null for non-swap.
5. "toToken": For swap action, identify the output token to convert TO (e.g., USDM, DJED). Set null for non-swap.
...
8. "poolTicker": For staking actions, extract the validator pool ticker (e.g. "CSP", "AINF") if mentioned. Otherwise set null.
...
12. "confidence": Set score based on prompt clarity.
`;

const BASE_SYSTEM_PROMPT = `You are a Base blockchain transaction parser AI.
Your ONLY output must be a valid JSON object. Do not include markdown codeblock syntax (no \`\`\`json tags), do not include any header comments, do not include greetings or follow-up remarks. Return ONLY the JSON object.

Extract transaction parameters from the user's natural language instruction.
Parse and construct a JSON block matching this EXACT schema:
{
  "action": "send" | "swap" | "recurring" | "schedule" | "stake" | "unstake" | "unknown",
  "amount": number,
  "token": "ETH" | "USDC" | "DEGEN" | "WETH" | string,
  "fromToken": "ETH" | "USDC" | "DEGEN" | "WETH" | string | null,
  "toToken": "ETH" | "USDC" | "DEGEN" | "WETH" | string | null,
  "receiverName": string | null,
  "receiverAddress": string | null,
...
1. "action":
   - "send" for direct transfers or payment schedules.
   - "swap" for token-to-token conversions (e.g. ETH to USDC).
   - "recurring" for ongoing subscriptions or continuous payments.
   - "schedule" for single delayed future transactions.
   - "stake" for depositing funds to a DeFi yield pool or vault.
   - "unstake" for withdrawing funds from vault.
2. "amount": Extract as a positive floating point number. If unspecified, default to 0.
3. "token": Identify the transacted token. Standard is ETH.
4. "fromToken": For swap action, identify the input token to convert FROM (e.g., ETH, USDC). Set null for non-swap.
5. "toToken": For swap action, identify the output token to convert TO (e.g., USDC, DEGEN). Set null for non-swap.
6. "receiverName": Extract contact name. For raw addresses, set this to null.
7. "receiverAddress": Extract raw EVM address (must start with 0x) only if the user typed one. Otherwise set null.
8. "poolTicker": For staking actions, extract the yield vault/protocol name (e.g. "Aave", "Aerodrome") if mentioned. Otherwise set null.
9. "transactionType": Match "transfer", "swap", "recurring", "scheduled", or "staking".
10. "schedule": If "every Friday", set "every Friday". Otherwise set null.
11. "riskLevel": Set "high" if prompt asks for suspicious actions. Set "medium" for unknown addresses. Otherwise default to "low".
12. "confidence": Set score based on prompt clarity.
`;

/**
 * Parses user input into transaction parameters using Groq
 * @param {string} text - The natural language intent
 * @param {string} blockchain - The active blockchain ("cardano" | "base")
 * @returns {Promise<object>} The parsed intent parameters
 */
export const parseIntent = async (text, blockchain = "cardano") => {
  try {
    ...
    return normalizeIntent(parsed, isBase);
  } catch (error) {
    console.error("Intent parsing parseError:", error.message);
    return {
      action: "unknown",
      amount: 0,
      token: blockchain === "base" ? "ETH" : "ADA",
      fromToken: null,
      toToken: null,
      receiverName: null,
...
const normalizeIntent = (intent, isBase = false) => {
  const defaultToken = isBase ? "ETH" : "ADA";
  const receiverName = intent.receiverName ?? (
    intent.receiver && !String(intent.receiver).startsWith("addr") && !String(intent.receiver).startsWith("0x") ? intent.receiver : null
  );
  ...
  return {
    ...
    token: String(intent.token || defaultToken).toUpperCase(),
    fromToken: intent.fromToken ? String(intent.fromToken).toUpperCase() : (intent.action === "swap" ? defaultToken : null),
